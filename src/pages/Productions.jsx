...
function Productions() {
  ...
  const handleSaveShow = async (formData) => {
    ...
    if (editingShow) {
      const { error } = await supabase
        .from("dbce_show")
        .update(showPayload)
        .match({ id: editingShow.id });
      if (error) {
        console.error("Error updating show:", error);
        setError("Failed to update show.");
      }
    } else {
      const { data: shows, error: numError } = await supabase
        .from("dbce_show")
        .select("show_number")
        ...
        .limit(1);
      if (numError) {
        console.error("Error fetching show number", numError);
      ...
      if (catError) {
        ...
        setError("Show created, but budget generation failed.");
        return;
      }

      const budgetItems = cats.map((c) => ({
        show_id: newShow.id,
        summary_group: c.summary_group,
      ...
      const { error: budgetError } = await supabase
        ...
        .insert(budgetItems);
      if (budgetError) {
        console.error("Error creating budget items", budgetError);
        setError("Show created, but budget generation failed.");
      }
    }
    fetchProductions();
  ...
  const openProductionModal = (prod = null) => {
  ...
  const openShowModal = (prodId, show = null) => {
        ...
        {productions.map((production) => (
          <div key={production.id} className="card production-card">
            <div className="card-header d-flex justify-content-between align-items-center">
              ...
              <div>
                <button
                  ...
                  onClick={() => openProductionModal(production)}
                >
                  <FaEdit />
                </button>
                <button
                  className="btn btn-danger btn-round btn-icon ms-2"
                  onClick={() => openConfirmModal(production, "production")}
                >
                  <FaTrash />
                </button>
              </div>
            ...
            <div className="card-body">
              <p className="mb-1">
                <strong>Agent:</strong> {production.agent || "N/A"}
              </p>
              <p>
                <strong>Email:</strong>{" "}
                {production.agent_email_address || "N/A"}
              </p>
              <hr />
                  ...
                  {production.shows.map((show) => (
                    <li key={show.id} className="show-item">
                      <span>
                        ...
                        {new Date(show.local_date).toLocaleDateString()}
                      </span>
                      <div className="show-item-buttons">
                        ...
                        <button
                          ...
                          onClick={() => openShowModal(production.id, show)}
                        >
                          <FaEdit />
                        </button>
                        <button
                          className="btn btn-danger btn-sm btn-icon"
                          onClick={() => openConfirmModal(show, "show")}
                        >
                          <FaTrash />
                        </button>
                      </div>
