...
function VenueSettings() {
  ...
  const [newVenue, setNewVenue] = useState({
    venue_name: '', city: '', total_capacity: '',
    seat_kills: '0', comps: '0', producer_holds: '0',
    venue_rental: '0', extra_show_fee: '0', hourly_extra_rate: '0'
  });
  ...
  const fetchVenues = async () => {
    const { data, error } = await supabase.from('dbce_venue').select('*').order('created_at', { ascending: false });
    if (error) {
  ...
  const validateVenue = (venue) => {
    ...
    const numericFields = ['total_capacity', 'seat_kills', 'comps', 'producer_holds', 'venue_rental', 'extra_show_fee', 'hourly_extra_rate'];
  ...
  const handleAddVenue = async () => {
    ...
    const { data, error } = await supabase.from('dbce_venue').insert([venueData]).select();
    if (error) {
  ...
  const handleUpdateVenue = async (id) => {
    ...
    const { data, error } = await supabase.from('dbce_venue').update(venueData).match({ id }).select();
    if (error) {
  ...
  const renderInputField = (key, placeholder) => (
    <input
      type={key.includes('capacity') || key.includes('kills') || key.includes('comps') || key.includes('holds') || key.includes('rate') || key.includes('fee') || key.includes('rental') ? 'number' : 'text'}
  ...
  const renderRow = (venue) => {
    ...
    return (
      <tr key={venue.id}>
        <td>{isEditing ? renderInputField('venue_name', 'Venue Name') : venue.venue_name}</td>
        <td>{isEditing ? renderInputField('city', 'City') : venue.city}</td>
        <td>{isEditing ? renderInputField('total_capacity', 'Total Capacity') : venue.total_capacity}</td>
        <td>{isEditing ? renderInputField('seat_kills', 'Seat Kills') : venue.seat_kills}</td>
        <td>{isEditing ? renderInputField('comps', 'Comps') : venue.comps}</td>
        <td>{isEditing ? renderInputField('producer_holds', 'Producer Holds') : venue.producer_holds}</td>
        <td>{totalSellable}</td>
        <td>{isEditing ? renderInputField('venue_rental', 'Venue Rental') : venue.venue_rental}</td>
        <td>{isEditing ? renderInputField('extra_show_fee', 'Extra Show Fee') : venue.extra_show_fee}</td>
        <td>{isEditing ? renderInputField('hourly_extra_rate', 'Hourly Extra Rate') : venue.hourly_extra_rate}</td>
        <td className="text-right">
          {isEditing ? (
            <>
              <button className="btn btn-success btn-round btn-icon" onClick={() => handleUpdateVenue(venue.id)}><FaSave /></button>
              <button className="btn btn-secondary btn-round btn-icon" onClick={() => setEditingVenue(null)}>Cancel</button>
            </>
          ) : (
            <>
              <button className="btn btn-warning btn-round btn-icon" onClick={() => startEditing(venue)}><FaEdit /></button>
              <button className="btn btn-danger btn-round btn-icon" onClick={() => handleDeleteVenue(venue.id)}><FaTrash /></button>
            </>
          )}
        </td>
      </tr>
  ...
  return (
    <div className="card">
      ...
      <div className="card-body">
        ...
        <button className="btn btn-primary btn-round mt-3" onClick={handleAddVenue}><FaPlus /> Add Venue</button>
